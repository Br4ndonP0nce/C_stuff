#ifndef PILA_ECHAURI_PILA_EST_H
#define PILA_ECHAURI_PILA_EST_H
#include <iostream>
...
//Size_t: Entero sin signo, se utiliza para indicar tama�os de colecciones
//typename: Declara el tipo de dato T que se puede ser cualquier tipo valido
class ListaEstatica{
private:
...
void ListaEstatica<n, T>::erase(size_t position)
{
    ...
    if (position >= index)
        throw range_error("Trying erase(size _t position) on non valid position");
    if (position==0)
        pop_front();
...
{
    if (empty())
        throw range_error("Trying [] in empty list");
    if (position >= index)
